...
def ensure_db():
    with _db() as conn:
        ...
        cur.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS tos_accepted_at TEXT DEFAULT ''")
        # Sessions table
        cur.execute("""
...
def db_create_user(username: str, password: str, display_name: str = "") -> str:
    ...
    with _db() as conn:
        cur = conn.cursor()
        cur.execute(
            "INSERT INTO users (id, username, password_hash, display_name, created_at, is_admin, tos_accepted_at) "
            "VALUES (%s,%s,%s,%s,%s,%s,%s)",
            (uid, username.lower().strip(), hash_password(password),
             display_name or username, _now(), is_admin, _now())
        )
    return uid
...
def delete_my_account(user: dict = Depends(get_current_user)):
    """Right to erasure (Amendment 13): user deletes their own account + all their data."""
    uid = user["id"]
    with _db() as conn:
        cur = conn.cursor()
        ...
        try:
            ...
            pass
        try:
            cur.execute("DELETE FROM pwa_installs WHERE user_id=%s", (uid,))
        except Exception:
            pass
...
<div class="card">
  ...
  <div class="foot">מופעל ע״י לידלי</div>
</div>
<script>
async function send(){
...
_PRIVACY_HTML = _legal_page("מדיניות פרטיות", (
    "<p>מדיניות זו מסבירה כיצד <b>לידלי</b> (\"השירות\", \"אנחנו\") אוסף, משתמש, "
    ...
    "<h2>1. מי אחראי על המידע (בעל שליטה ומעבד)</h2>"
    "<ul>"
    "<li><b>לגבי מידע החשבון שלך</b> (שם, שם משתמש, אימייל): מפעיל השירות הוא <b>בעל השליטה</b> במידע. "
    "פרטי יצירת קשר: kelrom10@gmail.com.</li>"
    "<li><b>לגבי נתוני הלידים שאתה מזין</b> (פרטי לקוחותיך): <b>אתה</b> בעל השליטה במידע, "
    "והשירות פועל עבורך כ<b>מעבד</b> — כלומר מעבד את המידע רק לפי הנחייתך ולצורך מתן השירות. "
    "כמפורט בסעיף העיבוד שבתנאי השימוש.</li>"
    "</ul>"

    "<h2>2. איזה מידע נאסף</h2><ul>"
    "<li><b>מידע חשבון:</b> שם תצוגה, שם משתמש, אימייל, וסיסמה השמורה בהצפנה חד-כיוונית.</li>"
    "<li><b>נתוני לידים שאתה מזין:</b> שם הלקוח, טלפון, מקור הפנייה, חבילה, תאריך אירוע, הערות ותגיות.</li>"
    "<li><b>מידע תפעולי:</b> מזהי התחברות (session), ומזהה צ'אט טלגרם אם חיברת — לצורך שליחת התראות.</li>"
    "</ul>"
    "<p>אנו פועלים לפי עקרון <b>מזעור מידע</b> — נאסף רק המידע הדרוש למתן השירות, ולא מעבר לכך.</p>"

    "<h2>3. מטרות השימוש והבסיס החוקי</h2><ul>"
    "<li>מתן השירות: ניהול לידים, תזכורות, יומן, סטטיסטיקות והתראות — על בסיס <b>הסכמתך</b> וקיום ההתקשרות.</li>"
    "<li>אבטחת החשבון ומניעת שימוש לרעה — על בסיס <b>אינטרס לגיטימי</b>.</li>"
    "</ul>"
    "<p>איננו מוכרים מידע אישי ואיננו משתמשים בו לפרסום צד שלישי.</p>"

    "<h2>4. שיתוף מידע וספקי משנה (Sub-Processors)</h2>"
    "<p>המידע עשוי להיות מעובד על ידי ספקי תשתית מהימנים, לצורך תפעול השירות בלבד:</p><ul>"
    "<li><b>Supabase</b> — אחסון מסד הנתונים.</li>"
    "<li><b>Render</b> — אירוח השרת והאפליקציה.</li>"
    "<li><b>Telegram</b> — משלוח התראות (רק אם חיברת חשבון).</li>"
    "<li>שירותי <b>Web Push</b> ודוא\"ל — למשלוח התראות ואיפוס סיסמה.</li>"
    "</ul>"
    "<p><b>העברה אל מחוץ לישראל:</b> חלק מהספקים מאחסנים מידע בשרתים מחוץ לישראל (לרוב באיחוד האירופי/ארה\"ב). "
    "השימוש בשירות מהווה הסכמה להעברה זו, הנעשית בכפוף לאמצעי אבטחה סבירים.</p>"

    "<h2>5. אבטחת מידע</h2>"
    "<p>אנו נוקטים אמצעי אבטחה סבירים בהתאם לרגישות המידע והיקפו, ובכללם: "
    "הצפנת תעבורה (HTTPS/TLS); שמירת סיסמאות בגיבוב חד-כיווני (PBKDF2); "
    "<b>בידוד מלא בין חשבונות</b> — כל משתמש רואה אך ורק את הנתונים שלו; "
    "אימות מבוסס token, הגבלת קצב על ניסיונות התחברות, וגיבוי נתונים תקופתי.</p>"

    "<h2>6. תקופת שמירה</h2>"
    "<p>המידע נשמר כל עוד החשבון פעיל ובמידה הדרושה למתן השירות. עם מחיקת החשבון — "
    "המידע (לרבות הלידים) נמחק. אינך מחויב למסור מידע, אך ללא מידע חשבון בסיסי לא ניתן לספק את השירות.</p>"

    "<h2>7. הזכויות שלך</h2>"
    "<p>בהתאם לחוק, עומדות לך הזכויות הבאות:</p><ul>"
    "<li><b>עיון</b> במידע שלך — דרך ייצוא הנתונים (Excel) שבאפליקציה.</li>"
    "<li><b>תיקון</b> מידע לא מדויק — דרך עריכה באפליקציה.</li>"
    "<li><b>מחיקה</b> — דרך \"מחיקת חשבון ומידע\" בתפריט, או בפנייה אלינו.</li>"
    "</ul>"
    "<p>למימוש זכויות או לכל שאלה: <b>kelrom10@gmail.com</b>.</p>"

    "<h2>8. אירוע אבטחה</h2>"
    "<p>במקרה של אירוע אבטחה חמור המשפיע על מידע אישי, נפעל בהתאם לחובות הדיווח שבדין ונודיע לנפגעים "
    "ולרשות להגנת הפרטיות ככל שנדרש.</p>"

    "<h2>9. עוגיות (Cookies)</h2>"
    "<p>השירות עושה שימוש באחסון מקומי (localStorage) לצורך שמירת התחברות והעדפות בלבד — "
    "לא לצורכי מעקב פרסומי.</p>"

    "<h2>10. שינויים ויצירת קשר</h2>"
    "<p>נעדכן מדיניות זו מעת לעת; המשך שימוש מהווה הסכמה לגרסה המעודכנת. לפניות בנושא פרטיות: "
    "<b>kelrom10@gmail.com</b>.</p>"
))

_TERMS_HTML = _legal_page("תנאי שימוש", (
    "<p>השימוש ב-<b>לידלי</b> (\"השירות\") כפוף לתנאים הבאים. שימוש בשירות מהווה הסכמה מלאה להם "
    "ולמדיניות הפרטיות.</p>"
    "<h2>1. השירות</h2>"
    "<p>השירות מספק כלי לניהול לידים, תזכורות, יומן והתראות לבעלי עסקי אירועים. אנו שואפים לזמינות גבוהה "
    "אך איננו מתחייבים לפעילות רציפה, ורשאים לעדכן או להשבית תכונות.</p>"
    "<h2>2. אחריות המשתמש</h2><ul>"
    "<li>אתה אחראי לשמירת סודיות פרטי הכניסה שלך ולכל פעילות בחשבונך.</li>"
    "<li>אתה אחראי לחוקיות המידע שאתה מזין, ולכך שיש לך <b>בסיס חוקי והסכמה</b> להחזיק ולעבד את פרטי הלקוחות (הלידים).</li>"
    "<li>בעת שליחת הודעות ללקוחות — עליך לפעול לפי הדין, לרבות דיני הדיוור (חוק הספאם) והסכמת הנמען.</li>"
    "<li>אין להשתמש בשירות למטרות בלתי חוקיות, פגיעה בפרטיות, או דיוור המוני לא מורשה.</li></ul>"

    "<h2>3. עיבוד מידע מטעמך (נספח עיבוד נתונים)</h2>"
    "<p>ביחס לנתוני הלקוחות שאתה מזין, <b>אתה בעל השליטה</b> והשירות משמש <b>מעבד</b> מטעמך. אנו מתחייבים:</p><ul>"
    "<li>לעבד את המידע רק לצורך מתן השירות ולפי הנחיותיך.</li>"
    "<li>לנקוט אמצעי אבטחה סבירים כמפורט במדיניות הפרטיות.</li>"
    "<li>להסתייע בספקי משנה (Supabase, Render וכו') הכפופים לחובות דומות.</li>"
    "<li>לאפשר לך לעיין, לתקן ולמחוק את המידע, ולמחוק את המידע עם סיום ההתקשרות.</li>"
    "<li>לסייע לך במידת הצורך במענה לפניות של נושאי מידע ובטיפול באירועי אבטחה.</li></ul>"

    "<h2>4. תשלומים ומנוי</h2>"
    "<p>ככל שיופעלו מסלולים בתשלום, התנאים והמחירים יוצגו מראש. ניתן לבטל מנוי בכל עת; "
    "הביטול ייכנס לתוקף בתום תקופת החיוב הנוכחית.</p>"

    "<h2>5. הגבלת אחריות</h2>"
    "<p>השירות ניתן \"כפי שהוא\" (AS-IS). איננו אחראים לנזק עקיף, אובדן רווחים או אובדן מידע; "
    "מומלץ לשמור גיבוי עצמאי (קיים ייצוא Excel). אחריותנו הכוללת מוגבלת לסכום ששולם עבור השירות ב-12 החודשים שקדמו לאירוע.</p>"

    "<h2>6. קניין רוחני</h2>"
    "<p>כל הזכויות בשירות ובקוד שייכות למפעיל. המידע שאתה מזין נותר בבעלותך.</p>"

    "<h2>7. שינויים וסיום</h2>"
    "<p>אנו רשאים לעדכן תנאים אלה מעת לעת; המשך שימוש מהווה הסכמה. ניתן לסגור חשבון בכל עת "
    "דרך \"מחיקת חשבון ומידע\" בתפריט.</p>"

    "<h2>8. דין וסמכות שיפוט</h2>"
    "<p>על תנאים אלה חל הדין הישראלי, וסמכות השיפוט הבלעדית נתונה לבתי המשפט המוסמכים בישראל.</p>"

    "<h2>9. יצירת קשר</h2><p>לשאלות: <b>kelrom10@gmail.com</b>.</p>"
))
...
<meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>לידלי · Leadly — לא לפספס אף ליד</title>
...
<div class="scaler"><div id="cap">
 ...
 <div class="foot">לידלי · ניהול לידים לאנשי אירועים</div>
</div></div>
